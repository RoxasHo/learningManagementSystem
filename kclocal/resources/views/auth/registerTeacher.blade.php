...
<body>
    <div class="registration-form">
        ...
        <form id="registerTeacherForm" action="{{ route('teacher.register') }}" method="POST" enctype="multipart/form-data">
            @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            form.addEventListener('submit', function(event) {
                ...
                if (!validateFileSizeAndType(document.getElementById('teacherPicture'), 2 * 1024 * 1024, ['image/jpeg', 'image/png'], document.getElementById('teacherPictureError'))) {
                    isValid = false;
                }
                ...
                if (isValid) {
                    ...
                    axios.post('{{ route('teacher.register') }}', formData)
                        .then(response => {
            ...
            form.querySelectorAll('input, select, textarea').forEach(input => {
                input.addEventListener('blur', function() {
                    ...
                    axios.post('{{ route('teacher.validate') }}', formData)
                        .then(response => {
